...
public class MessagePackCodec {
    ...
    private void encodeCommand(final Command command, final MessagePacker messagePacker) throws IOException {
        if (command instanceof ValueChangedCommand) {
            ...
            encodeDeleteBeanCommand((DeleteBeanCommand) command, messagePacker);
        } else if (command instanceof ListAddCommand) {
            encodeListAddCommand((ListAddCommand) command, messagePacker);
        } else if (command instanceof ListReplaceCommand) {
            encodeListReplaceCommand((ListReplaceCommand) command, messagePacker);
        } else {
            throw new RuntimeException("Unknown ccommand type");
        }
    }
    ...
    private Command decodeCommand(final MessageUnpacker messageUnpacker) throws IOException {
        final int propertyCount = messageUnpacker.unpackMapHeader();
        final KeyValue typeTuple = unpack(messageUnpacker);
        final KeyValue idTuple = unpack(messageUnpacker);
        final String type = typeTuple.getValueAsString();
        if(Objects.equals(LIST_ADD_COMMAND_ID, type)) {
            return decodeListAddCommand(messageUnpacker, idTuple.getValueAsString(), propertyCount - 2);
        } else {
    ...
    public List<Command> decode(final String transmitted) throws CodecException {
        try {
            final MessageUnpacker messageUnpacker = MessagePack.newDefaultUnpacker(ConnectionUtils.fromBase64(transmitted));
            final int commandCount = messageUnpacker.unpackArrayHeader();
            return IntStream.of(commandCount)
                    .mapToObj(i -> {
                        try {
                            return decodeCommand(messageUnpacker);
                        } catch (IOException e) {
                            throw new RuntimeException("Error in decode", e);
                        }
                    })
                    .collect(Collectors.toList());
        } catch (Exception e) {
            throw new CodecException("Error in decode", e);
        }
    }
