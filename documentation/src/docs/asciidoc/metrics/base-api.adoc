...
All factory methods for metrics in the `dev.rico.metrics.Metrics` interface will only create a new metric of no metric with the given name already exists.
Otherwise the existing metric will be returned.
Using the unique name of the metric you can receive all values of the metric for monitoring and maintenance issues later.
